...
#     docker build -t scalems/workshop .
#
# Run the jupyter notebook server
#     docker run --rm -ti -p 8888:8888 scalems/workshop
#
# Run an interactive shell:
#     docker run --rm -ti scalems/workshop bash
#     . venv/bin/activate
#     cd examples
#     python
#     >>> ...
...
ARG GMXAPI_REF="gmxapi==0.3.1"

RUN . $VENV/bin/activate && \
    . /usr/local/gromacs/bin/GMXRC && \
    pip install --no-cache-dir $GMXAPI_REF && \
    python -c 'import gmxapi'

ENV PROJECT_DIR /home/tutorial/scalems-workshop
ADD --chown=tutorial:tutorial input_files $PROJECT_DIR/input_files
#ADD --chown=tutorial:tutorial examples $PROJECT_DIR/examples
ADD --chown=tutorial:tutorial notebooks $PROJECT_DIR/notebooks
